...
\begin{multicols}{2}
	...
	\noindent \NewsItem{Correct predictions}
	
	\noindent In all, we had five correct predictions from the aforementioned folks:
...
\end{multicols}
	\vspace{0.5cm}
	\noindent \NewsItem{Surprise finds}
	\noindent While likely discussed, the following species were not in folks' top tens for predicted next species in Nebraska--and it seems very unlikely that Nebraska will get a second record of several of these anytime soon!
	\begin{multicols}{2}
	\begin{enumerate}[resume]
	...
	\begin{enumerate}[resume]
		...
		While there were several possible and probably records of this unique thicket- and bamboo-loving warbler in the state, the species was not confirmed until a singing bird turned up in Wilderness Park, \textit{Lancaster} for a single day. There has been only one other record of the species since---a cooperative bird filmed in \textit{Polk} during migration.
		...
		\begin{enumerate}[resume]
		\item \textbf{Brown Booby \textit{Sula leucogaster}}
	\end{enumerate}
	An early summer record of this pelagic seabird was found at Harlan County Reservoir during a fishing trip, and stayed around for a while (much to the enjoyment of Nebraska's birders). Given recent records from other nearby states, it seems possible that this species will show up again in the future.
	
	\end{multicols}
	
	\noindent \NewsItem{Predictions yet to show}
	\noindent The following species were predicted in past years, but have yet to show in Nebraska. 
...
\begin{multicols}{2}
	\noindent \NewsItem{Had to have been here!}
	\noindent Multiple species not presently on the Nebraska state list have likely passed through the state, based on records from adjacent regions. Some of these species seem more likely than others, but all are worth mentioning!
	\begin{itemize}
		\item \textbf{Barnacle Goose \textit{Branta leucopsis}}
	\end{itemize}
	...
	\end{itemize}
	Records of this dapper sandpiper exist from Texas and northeast Kansas.
	...
	\end{itemize}
	A record for this handsome woodpecker on 6 December 2006 was only ca 2.4 miles from the Nebraska state line in east-central Iowa, indicating this bird almost certainly flew through Nebraska airspace.
	...
	\begin{itemize}
		\item \textbf{Amur Stonechat \textit{Saxicola stejnegeri}}
	\end{itemize}
...
\end{multicols}
	...
	\vspace{0.5cm}
	\section*{What do we think will be next?}
	\begin{multicols}{2}
	
	\noindent \NewsItem{Tobin Brown}
	
	\noindent Tobin is the current big year record holder for the state of Nebraska.
	
	\begin{etaremune}
		\item Spotted Redshank
		\item Black-backed Woodpecker
		\item Mexican Violetear
		\item Lesser Nighthawk
		\item Fork-tailed Flycatcher
		\item Tropical Kingbird
		\item Bushtit
		\item Greater Roadrunner
		\item Slaty-backed Gull
		\item Vega Gull
	\end{etaremune}
	
	\noindent \NewsItem{Keith Geluso}
	
	\noindent Keith Geluso is a Professor at the University of Nebraska at Kearney, and the one-time record-holder of the Big Day record for the state of Nevada.
	
	\begin{etaremune}
		\item Canyon Towhee
		\item Rufous-crowned Sparrow
		\item Boreal Chickadee
		\item Long-billed Thrasher
		\item Bronzed Cowbird
		\item Juniper Titmouse
		\item Woodhouse's Scrub-Jay
		\item Greater Roadrunner
		\item Carolina Chickadee
		\item Bushtit
	\end{etaremune}
	
	\noindent \NewsItem{University of Nebraska at Kearney Ornithology}
	
	\noindent This contentious list was argued over \textit{ad naseum} by Jacob C. Cooper, Eugene Huryn, Cody McGregor, Peggy Huss, Sawyer Therrien, Connor Harrison, and Letty Reichart.
	
	\begin{etaremune}
		\item Pyrrhuloxia
		\item Lesser Nighthawk
		\item Greater Roadrunner
		\item Wilson's Plover
		\item Painted Whitestart
		\item Mexican Violetear
		\item Slaty-backed Gull
		\item Purple Sandpiper
		\item Fork-tailed Flycatcher
		\item Tropical Kingbird
	\end{etaremune}
